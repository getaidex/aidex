...
|8|`08-resume-analyzer`|3|`resume.analyze` engine on a sample resume.md fixture. Real output fields (verified against source, not the original wishlist): `candidateName?, skills, experienceYears?, summary?, matchScore?` — no `strengths`/`weaknesses`/`atsScore` field exists. Example prompts for an optional job description so `matchScore` (0-100, only populated when a JD is supplied) has something meaningful to compare against|new|
