...
describe('callProviderWithObservability', () => {
  it('returns the call()s response unchanged', async () => {
    ...
    const result = await callProviderWithObservability({
      strategyName: 's',
      providerName: 'p',
      call: async () => response,
    });
  ...
  it('records tokens when usage metadata is present but pricing is not configured', async () => {
    ...
    const events = observability.getTimeline();
    expect(events.map((e) => e.event)).toEqual(['provider', 'duration', 'tokens']);
    expect(events[2].metadata).toMatchObject({ provider: 'p', strategy: 's', inputTokens: 10, outputTokens: 5 });
  });
  ...
  it('ignores malformed usage metadata rather than throwing', async () => {
    const observability = new ObservabilityBus();
    ...
    await callProviderWithObservability({
      ...
      providerName: 'p',
      observability,
      call: async () => ({ content: 'ok', metadata: { usage: 'not an object' } }),
    });
  ...
  it('records provider (failure) + duration + error and rethrows on a rejected call', async () => {
    ...
    expect(events[2].metadata).toMatchObject({ error });
  });
