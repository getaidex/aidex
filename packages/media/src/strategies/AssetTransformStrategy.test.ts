...
import { AssetTransformStrategy } from './AssetTransformStrategy.js';
...
const VALID_RESPONSE = JSON.stringify({ description: 'Converted to black and white with high contrast' });

describe('AssetTransformStrategy', () => {
  it('exposes its name and version', () => {
    const strategy = new AssetTransformStrategy(makePrompts());
    expect(strategy.name).toBe('media-asset-transform');
    expect(strategy.version).toBe('1.0.0');
  });

  it('parses a valid JSON response and keeps the source mimeType', async () => {
    const provider: Provider = { name: 'inline', async generate() { return { content: VALID_RESPONSE }; } };
    const strategy = new AssetTransformStrategy(makePrompts());

    const result = await strategy.execute(
      { strategy: strategy.name, input: { brief: 'Make it black and white', source: SOURCE } },
