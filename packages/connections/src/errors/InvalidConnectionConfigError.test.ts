...
describe('InvalidConnectionConfigError', () => {
  it('carries the connection id, reason, and a descriptive message', () => {
    const error = new InvalidConnectionConfigError('conn-1', 'config must be a non-null object');
    ...
    expect(error.reason).toBe('config must be a non-null object');
    expect(error.message).toBe('Invalid configuration for connection "conn-1": config must be a non-null object');
